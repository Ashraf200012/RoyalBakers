...
query = f""" SELECT * from ordermaster"""    
#print(query)
...
for x in myresult:
    ...
    if x[3] == "Pending":
        accept_btn = f'<a href="orderAccept.py?id={x[0]}"><button>Accept Order</button></a>'

    tr_html += f"""
    <tr>
        <td>
            {accept_btn}
            <a href="ViewOrders.py?id={x[0]}"><button>View Order Details</button></a>
        </td>
        <th scope="row">{x[0]}</th>
        <td>{x[2]}</td>
        <td>{x[3]}</td>
